...
LookSee는 환경 변수로 구성합니다. Docker Compose에서는 `compose.yaml` 옆의 `.env`에서 읽으며, `.env.example`이 모든 변수를 기본값과 함께 나열합니다. 서비스는 시작할 때 변수를 읽으므로 변경 사항은 `docker compose up -d` 뒤에 적용됩니다.
...
| `MTX_MEDIA_PASSWORD` | 필수 | MediaMTX 미디어 사용자의 비밀번호. Studio를 불러오는 브라우저에 노출됩니다. |
| `MTX_MEDIA_USER` | `media` | 읽기와 송출 권한이 있는 MediaMTX 사용자. |
...
다른 서비스에도 제한이 있습니다. `api`는 CPU 2개와 1 GB, `postgres`는 CPU 2개와 2 GB, `mediamtx`는 CPU 2개와 1 GB, `redis`는 CPU 1개와 768 MB, `studio`는 CPU 1개와 512 MB입니다. 바꾸려면 `compose.yaml`을 편집하거나 override 파일을 추가합니다.
...
| `CORS_ORIGIN_REGEX` | `^https?://(localhost\|127\.0\.0\.1)(:\d+)?$` | 브라우저에서 API를 호출할 수 있는 오리진. Studio가 localhost가 아니면 Studio 오리진으로 설정합니다. |
| `EVENT_COOLDOWN_SECONDS` | `2` | 같은 카메라에서 같은 종류의 두 이벤트 사이의 최소 간격. `0`은 재알림 대기 시간을 비활성화합니다. |
...
| `S3_ENDPOINT_URL`, `S3_BUCKET` | 비어 있음 | File 카메라용 자산 라이브러리. 둘 다 설정해야 활성화됩니다. |
| `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` | 비어 있음 | 자산 라이브러리 자격 증명. |
| `S3_REGION` | `auto` | 리전. Cloudflare R2는 `auto`. |
...
| `RUNTIME_MEDIAMTX_MEDIA_USER`, `RUNTIME_MEDIAMTX_MEDIA_PASSWORD` | MediaMTX 미디어 사용자 | 재생과 웹캠 송출을 위해 브라우저에 전송됩니다. |
| `RUNTIME_DOCS_URL` | `http://<WEBRTC_HOST_IP>:3002/docs` | 사이드바 **Documentation**(문서) 링크의 대상. |
...
| 볼륨 | 서비스 | 내용 |
| --- | --- | --- |
| `postgres_data` | postgres | 워크플로, 카메라, 자격 증명, 알림, 사용자 |
...
`postgres_data`와 `api_keys`가 백업할 가치가 있는 상태를 담습니다. 시크릿이 없으면 저장된 자격 증명을 해독할 수 없고 모든 세션이 로그아웃됩니다. [배포](deployment.md#백업)에서 백업 절차를 설명합니다.
