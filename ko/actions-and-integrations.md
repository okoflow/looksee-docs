...
선택한 메서드(기본 `POST`, 또는 `GET`이나 `PUT`)로 이벤트를 JSON으로 URL에 전송합니다. 자격 증명은 필요 없습니다. 수신 측이 토큰을 요구하면 URL에 넣습니다. 요청은 5초 후 시간 초과됩니다. `5xx`, `408`, `429` 응답은 재시도되고 다른 오류는 재시도되지 않습니다. 각 요청은 `Idempotency-Key` 헤더에 전달 ID를 담습니다. 페이로드는 [API](api.md#웹훅-페이로드)에 문서화되어 있습니다.
...
- Alert와 Snapshot은 자신에게 도달한 모든 이벤트에 대해 그래프 순서로 즉시 실행됩니다. Webhook, Telegram, Discord, Slack, Email, MQTT는 PostgreSQL의 전달 큐에 들어가 API 안의 워커가 보내므로, 느린 수신 측이 탐지를 지연시키지 않습니다.
- 일시적인 이유(타임아웃, HTTP `408`, `429`, `5xx`)로 실패한 전달은 시도마다 두 배로 늘어나는 간격으로 최대 5분까지, 최대 8회 재시도됩니다. 다른 실패는 최종입니다. 메시지는 최소 한 번 전달되며, `GET /deliveries`가 큐를 나열합니다.
- 실패한 액션은 그 이벤트의 다른 액션을 멈추지 않으며, 두 분기가 같은 액션으로 이어지더라도 각 액션은 이벤트당 최대 한 번 실행됩니다.
