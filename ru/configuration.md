...
берутся из `.env` рядом с `compose.yaml`; `.env.example` перечисляет
переменные, которые задаёт установка, сгруппированные по сервисам, а
остальные с их значениями по умолчанию приведены в таблицах ниже. Сервисы
читают переменные при запуске, поэтому изменение вступает в силу после
`docker compose up -d`.
...
| `MTX_MEDIA_PASSWORD` | обязательна | Пароль служебного пользователя MediaMTX, общий для API, сервиса инференса и MediaMTX. Браузерам никогда не передаётся. |
| `MTX_MEDIA_USER` | `media` | Служебный пользователь MediaMTX с правами чтения и публикации. |
| `STORAGE_PASSWORD` | обязательна | Секретный ключ встроенного видеохранилища; ключ доступа — `looksee`. |
| `STORAGE_PORT` | `9000` | Порт хоста для S3 API хранилища, привязан к `127.0.0.1`. |
...
| `MTX_AUTHHTTPADDRESS` | `http://api:8000/internal/media/auth` | Куда MediaMTX обращается за авторизацией. Меняйте только когда API работает вне compose. |
...
и 2 ГБ, `mediamtx` — 2 CPU и 1 ГБ, `storage` — 2 CPU и 1 ГБ, `redis` —
1 CPU и 768 МБ, `studio` — 1 CPU и 512 МБ. Чтобы изменить их, отредактируйте `compose.yaml` или добавьте
...
| `CORS_ORIGIN_REGEX` | `^https?://(localhost\|127\.0\.0\.1)(:\d+)?$` | Источники (origin), которым разрешено обращаться к API из браузера. Запросы, изменяющие состояние, и подключения WebSocket с других источников отклоняются. Задайте origin Studio, когда это не localhost. |
| `EVENT_COOLDOWN_SECONDS` | `2` | Минимальный промежуток между двумя сообщениями `event` одного типа на одной камере в живой ленте. `0` отключает паузу. В граф попадает каждое событие. |
...
| `S3_ENDPOINT_URL`, `S3_BUCKET` | `http://storage:9000`, `looksee` | Библиотека файлов для камер File. Compose направляет их на встроенное хранилище; задайте обе, чтобы использовать внешний S3-совместимый бакет. При нативном запуске для включения библиотеки нужны обе. |
| `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` | `looksee`, `STORAGE_PASSWORD` | Учётные данные библиотеки файлов. |
| `S3_REGION` | `auto` | Регион внешнего бакета; `auto` для Cloudflare R2. |
...
| `RUNTIME_DOCS_URL` | `https://github.com/okoflow/looksee-docs` | Адрес ссылки **Documentation** в боковой панели; compose закрепляет его за репозиторием документации. |
...
| `9000` | storage | `127.0.0.1` | S3 API видеохранилища |
...
| --- | --- | --- |
| `postgres_data` | postgres | Рабочие процессы, камеры, учётные данные, оповещения, пользователи, очередь доставок |
| `storage_data` | storage | Загруженные видеофайлы для камер File |
...
`postgres_data`, `api_keys` и `storage_data` хранят состояние, которое стоит
резервировать; без секрета сохранённые учётные данные невозможно
расшифровать, а все сессии завершаются. Раздел [Развёртывание](deployment.md#резервные-копии) описывает
