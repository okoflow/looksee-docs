...
**severity** (`info`, `warning`, `critical`) и **cooldown** в секундах (от 0
...
поместите его в URL. Запрос прерывается через пять секунд, ответ не из
диапазона 2xx записывается в журнал без повторной отправки. Тело запроса
описано в разделе [API](api.md#тело-запроса-webhook).
...
**topic** (по умолчанию `looksee/events`) и **payload template**. Пустой
шаблон публикует тот же JSON, что и вебхук; заполненный — свой
отрисованный текст. Соединения прерываются через пять секунд.

## Поведение доставки

- Действия выполняются в порядке графа для каждого события, которое до них
  доходит. Действие, завершившееся ошибкой, записывает её в журнал и не
  останавливает остальные действия.
- Каждое действие выполняется не более одного раза на событие, даже когда к
  нему ведут две ветки.
