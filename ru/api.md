...
Все конечные точки, кроме `/health`, `/auth/*`, `/docs`, `/openapi.json` и
обратного вызова MediaMTX `/internal/media/auth`, требуют cookie
`looksee_session`. Получите его, войдя в систему, и используйте семь дней;
`/auth/login` и `/auth/setup` принимают 20 попыток с одного клиента и 100
суммарно в минуту, затем отвечают `429`:
...
| `403` | Заголовок `Origin` не подходит ни под `CORS_ORIGIN_REGEX`, ни под собственный origin API |
| `404` | Неизвестный рабочий процесс, камера, оповещение, учётные данные, доставка или файл |
| `409` | Учётная запись владельца уже существует; повтор доставки, которая не завершилась сбоем |
| `422` | Проверка не пройдена: поле вне допустимого диапазона, или граф не может работать (задан `code`) |
| `429` | Слишком много попыток входа; подождите `Retry-After` |
| `503` | Видеохранилище не настроено или недоступно; вход, пока недоступен Valkey |
...
| Метод и путь | Описание |
| --- | --- |
| `POST /cameras/{camera_id}/media-access` | `action` — `read` или `publish`; возвращает `token` для MediaMTX, действительный пять минут. Публикация выдаётся только для камер Browser webcam. |
| `POST /internal/media/auth` | Вызывается MediaMTX для каждого подключения; отвечает `204` или `401`. Не для клиентов. |
...
Сообщения Webhook, Telegram, Discord, Slack, Email и MQTT, поставленные
графом в очередь; политика повторов описана в разделе
[Мониторинг и оповещения](monitoring-and-alerts.md#доставки).

| Метод и путь | Описание |
| --- | --- |
| `GET /deliveries` | Начиная с самых новых: `id`, `status` (`pending`, `processing`, `sent`, `failed`), `attempts`, `available_at`, `last_error`, `created_at`. Фильтры: `status`, `limit` (от 1 до 100, по умолчанию 50) |
| `POST /deliveries/{id}/retry` | Поставить неудавшуюся доставку в очередь заново; возвращает `204` или `409`, если она не завершилась сбоем |

### Файлы

Доступны, когда настроена библиотека файлов, что стек compose делает по
умолчанию; пока хранилище недоступно, каждый вызов возвращает `503`.
...
| `event` | `kind`, `ts`, `frame_width`, `frame_height`, `detections[]` | Каждое событие, которое получает граф, не чаще одного на тип и камеру в пределах `EVENT_COOLDOWN_SECONDS` |
